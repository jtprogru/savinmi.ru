...
- `scripts/` — вспомогательные скрипты (снапшот зависимостей для GitHub)
...
Граф зависимостей GitHub не читает `bun.lock`, поэтому дерево отдаётся ему явно — `make dependency-snapshot` собирает снапшот, а CI отправляет его через Dependency Submission API. Без этого в графе видны только прямые зависимости и не приходят алерты по транзитивным CVE.
